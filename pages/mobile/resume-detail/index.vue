    ...
    <a-row type="flex" justify="space-between" class="resume">
      <div class="resume_left">
        <div class="centent">
          ...
          <div id="video">
            ...
            <a-row
              ...
              align="top"
              v-if="Object.keys(desVideo).length > 0"
            >
              <div class="video">
                <video width="100%" height="100%" controls>
                  <source :src="desVideo.video_link" type="video/mp4" />
                  <source :src="desVideo.video_link" type="video/ogg" />您的浏览器不支持 HTML5 video 标签。
                </video>
              </div>
              <div v-if="isEdit  && isShow" class="btn">
                ...
                <a>
                  ...
                  <span @click="del('video', desVideo.id)">删除</span>
                </a>
              </div>
            </a-row>
          </div>
          ...
          <div id="collection">
            ...
            <div class="picture">
              <div v-for="(item, index) in collection.collection_link" :key="index">
                <img :src="item" alt />
                <a v-if="isEdit && isShow" @click="delCollection(index)">
                  <a-icon type="delete" />
                </a>
              </div>
            </div>
        ...
        <div class="foot flexCenter" v-if="!isEdit && isShow">
          <a-button
            v-if="!sendStatus.is_interview&&sendStatus.is_read!=2"
            @click="addMace"
            type="primary"
          >邀请面试</a-button>
          <a-button
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.isEdit =
        userInfo.id == this.$route.query.user_id && userInfo.type == 2;
    ...
    getData() {
      ...
      ajax.get("user/resume/detail", query).then(res => {
        ...
        if (_data) {
          ...
          if (this.collection.collection_link)
            this.collection.collection_link = JSON.parse(
              this.collection.collection_link
            );
          if (_data.sendStatus) {
    ...
    getAnnexResumes() {
      ajax
        .get("user/annexResumes/list", { user_id: this.user_id })
        .then(res => {
          if (res.retcode == 0) {
